    ...
    class UnhandledExceptionDlg
    {
        ...
        private void ShowUnhandledExceptionDlg(Exception e)
        {
            ...
            try
            {
                ...
                exDlgForm.checkBoxRestart.CheckedChanged += delegate
                    {
                        _dorestart = exDlgForm.checkBoxRestart.Checked;
                    };

                // Handle clicks on report link label
                exDlgForm.linkLabelData.LinkClicked += delegate
                    {
                        if (OnShowErrorReport == null) return;
                        SendExceptionClickEventArgs ar = new SendExceptionClickEventArgs(true, unhandledException,
                                                                                         _dorestart);
                ...
                if (OnSendExceptionClick == null) return;

                SendExceptionClickEventArgs sendException = new SendExceptionClickEventArgs(sendDetails, unhandledException, _dorestart);
                OnSendExceptionClick(this, sendException);
            }
            finally
            {
                exDlgForm.Dispose();
